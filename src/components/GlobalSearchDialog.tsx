...
import { MagnifyingGlassIcon, XMarkIcon, CheckCircleIcon, ClockIcon, CalendarIcon } from '@heroicons/react/24/outline';
                ...
                >
                  ...
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0">
                    {result.type === 'task' ? (
                      ...
                      >
                        {result.item.icon === 'star' ? '⭐' : result.item.icon === 'heart' ? '❤️' : result.item.icon === 'fire' ? '🔥' : '📌'}
                      </div>
                    ) : (
                      <div
                        className="w-6 h-6 rounded-lg flex items-center justify-center text-xs"
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">
                      {result.type === 'task' ? result.item.title : result.type === 'habit' ? result.item.name : result.item.title}
                    </div>
                    <div className="flex items-center gap-2 text-xs text-gray-400 dark:text-gray-500">
                      <span className={`px-1.5 py-0.5 rounded-full ${
                        ...
                        {result.type === 'task' ? t('search.type_task') : result.type === 'habit' ? t('search.type_habit') : t('search.type_countdown')}
                      </span>
