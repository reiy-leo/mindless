...
function buildCalendarDays(year: number, month: number) {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    days.push({ dateStr: toDateStr(year, month, d), day: d, inMonth: true })
  }
    ...
    <>
      ...
      <div className="mt-2 pt-2 border-t border-gray-100 dark:border-gray-700 grid grid-cols-2 gap-2">
        <div>
          ...
          {startLunar && (
            <span className="text-xs text-gray-600 dark:text-gray-300">
              {startLunar.yearStr} {startLunar.monthStr}
              {startLunar.dayStr}
        ...
        <div>
          ...
          {endLunar && (
            <span className="text-xs text-gray-600 dark:text-gray-300">
              {endLunar.yearStr} {endLunar.monthStr}
              {endLunar.dayStr}
      ...
      <div className="mt-2 pt-2 border-t border-gray-100 dark:border-gray-700 grid grid-cols-2 gap-3">
        <div>
          <label
            ...
            htmlFor="time-pickers"
            aria-labelledby="time pickers"
          >
        ...
        <div>
          <label
            className="text-xs text-gray-500 dark:text-gray-400 block mb-1"
            htmlFor="time-pickers2"
            aria-labelledby="time pickers2"
          >
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      >
        ...
        <span
          className={`text-sm truncate ${startDate || endDate ? 'text-gray-900 dark:text-gray-100' : 'text-gray-400 dark:text-gray-500'}`}
        >
          {triggerText}
        </span>
      </button>
      ...
      {open && (
        <div
          className="absolute z-50 mt-1 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 p-3"
          style={{ maxWidth: '620px', minWidth: '560px' }}
        >
          {calendarContent}
        </div>
      )}
