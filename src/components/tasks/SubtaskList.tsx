        ...
        {isEditing ? (
          ...
          <span
            ...
            className={`flex-1 text-sm cursor-text ${
              subtask.isCompleted ? 'line-through text-gray-400' : 'text-gray-900'
            }`}
          >
            {subtask.title}
          </span>
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-medium text-gray-700">{t('tasks.subtasks.title')}</h3>
        <button
          onClick={() => onAdd()}
      ...
      {subtasks.length === 0 ? (
        <p className="text-sm text-gray-400 italic">{t('tasks.subtasks.empty')}</p>
      ) : (
        <div className="space-y-2">
