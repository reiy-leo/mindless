  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-medium text-gray-700">{t('tasks.steps.title')}</h3>
        <button
          onClick={onAdd}
      ...
      {steps.length === 0 ? (
        <p className="text-sm text-gray-400 italic">{t('tasks.steps.empty')}</p>
      ) : (
        ...
        {isEditing ? (
          ...
          <p
            ...
            className={`text-sm cursor-text ${
              step.isCompleted ? 'line-through text-gray-400' : 'text-gray-900'
            }`}
          >
            {step.description}
          </p>
            ...
            {showDatePicker && (
              <>
                ...
                <input
                  ...
                  className="absolute top-full left-0 mt-1 z-20 px-2 py-1 text-sm border border-gray-300 rounded shadow-lg bg-white"
                  autoFocus
                />
              </>
            )}
