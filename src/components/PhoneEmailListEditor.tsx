...
import { Check, ExternalLink, Mail, Phone, Plus, Trash2, User } from 'lucide-react'
import { useEffect, useRef, useState } from 'react'
import type { EmailEntry, PhoneEntry } from '@/types/person'
...
export default function PhoneEmailListEditor({
  type,
  entries,
  onChange,
  showAddForm: externalShow,
  setShowAddForm: externalSetShow,
  labelPresets,
  displayMode = 'list',
}: PhoneEmailListEditorProps) {
  ...
  const defaultLabel =
    type === 'phone' ? '手机' : type === 'email' ? '邮箱' : type === 'watch_link' ? '在线观看' : '别名'
          ...
          {entries.map((entry) => (
            ...
            >
              {isLinkType ? (
                ...
                >
                  {entry.value}
                  {copiedId === entry.id ? <Check className="w-3 h-3 text-green-500 shrink-0" /> : null}
                </button>
              ) : (
                <span className="text-sm text-gray-900 dark:text-gray-100 flex-1 truncate min-w-0">{entry.value}</span>
              )}
              ...
              <button
                ...
                className="p-0.5 rounded hover:bg-red-100 dark:hover:bg-red-900/30 text-red-400 opacity-0 group-hover:opacity-100 transition-opacity shrink-0"
              >
          ...
          <div key={entry.id} className="flex items-center gap-2 group">
            ...
            {isLinkType ? (
              <div className="flex-1 min-w-0 flex items-center gap-1.5">
                {entry.note && <span className="text-sm text-gray-900 dark:text-gray-100 truncate">{entry.note}</span>}
                <button
                  type="button"
                  onClick={() => handleOpenLink(entry.value)}
                  className="text-xs text-blue-500 hover:text-blue-600 dark:text-blue-400 dark:hover:text-blue-300 truncate flex items-center gap-0.5 shrink-0"
                >
              ...
              <span className="text-sm text-gray-900 dark:text-gray-100 flex-1 truncate">{entry.value}</span>
            )}
            {!isLinkType && entry.label && entry.label !== defaultLabel && (
            ...
            {!isLinkType && entry.label === defaultLabel && entry.note && (
              <span className="text-xs text-gray-400 dark:text-gray-500 truncate">({entry.note})</span>
            )}
            <button
              type="button"
              onClick={() => handleDelete(entry.id)}
