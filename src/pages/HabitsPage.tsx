...
function isHabitDueOnDate(habit: Habit, dateStr: string): boolean {
  ...
  switch (habit.frequency) {
    case 'daily':
      return true
    case 'every_x_days': {
      const interval = habit.frequencyDays ? parseInt(habit.frequencyDays, 10) : 1
...
function HabitFormDialog({
  ...
  const [everyXDays, setEveryXDays] = useState(() => {
    if (habit?.frequency === 'every_x_days' && habit.frequencyDays) {
      return parseInt(habit.frequencyDays, 10) || 2
    }
    return 2
  })
  ...
  useEffect(() => {
    if (isOpen && habit) {
      ...
      setEveryXDays(
        habit.frequency === 'every_x_days' && habit.frequencyDays ? parseInt(habit.frequencyDays, 10) || 2 : 2,
      )
      ...
      <div className="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-lg mx-4 max-h-[90vh] flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-auto p-6 space-y-5">
          ...
          <div className="space-y-3">
            ...
            <div className="border border-gray-300 dark:border-gray-600 border-none overflow-hidden min-h-15">
              <MilkdownEditor markdown={description} onChange={setDescription} placeholder={t('habits.description')} />
            </div>
          </div>
          ...
          <div>
            ...
            <Select
              ...
              onChange={setGroupId}
              options={[
                { label: t('habits.groups.all'), value: '' },
                ...habitGroups.map((g) => ({ icon: g.icon || '📁', label: g.name, value: g.id })),
              ]}
            />
          </div>
          ...
          <div>
            ...
            <div className="flex gap-1.5 flex-wrap">
              {[
                { label: t('habits.frequency.daily'), value: 'daily' },
                { label: t('habits.frequency.every_x_days'), value: 'every_x_days' },
                { label: t('habits.frequency.weekly'), value: 'weekly' },
                { label: t('habits.frequency.monthly'), value: 'monthly' },
              ].map((opt) => (
                <button
                  key={opt.value}
                  type="button"
          ...
          {frequency === 'every_x_days' && (
            <div className="flex items-center gap-3">
              ...
              <input
                ...
                max={365}
                value={everyXDays}
                onChange={(e) => setEveryXDays(Math.max(2, parseInt(e.target.value, 10) || 2))}
            ...
            <div className="flex items-center gap-3">
              <div className="flex-1">
                <label
                  ...
                  htmlFor="target value and unit"
                  aria-labelledby="target value and unit"
                >
...
function CheckInCalendar({ habit }: { habit: Habit }) {
  ...
  const stats = useMemo(() => {
    ...
    return { checkedDays, dueDays, missedDays, rate }
    ...
    <div className="mt-4 border border-gray-100 dark:border-gray-700 rounded-xl p-3 bg-gray-50/50 dark:bg-gray-800/50">
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-1">
          ...
          <span className="text-sm font-semibold text-gray-800 dark:text-gray-200 min-w-32.5 text-center">
            {monthLabel}
          </span>
          <button
            type="button"
          ...
          return (
            ...
            >
              ...
              {isChecked && log && habit.targetType !== 'binary' && (
                <span
                  className={`text-xs leading-tight font-medium ${fillOpacity > 0.5 ? 'text-white/80' : ''}`}
      ...
      {selectedDay && (
        <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-600">
          ...
          {selectedLog?.note && <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{selectedLog.note}</p>}
        </div>
      )}
...
function HabitCard({
  ...
  const getFrequencyLabel = (freq: HabitFrequency) => {
    switch (freq) {
      ...
      case 'every_x_days': {
        const n = habit.frequencyDays ? parseInt(habit.frequencyDays, 10) : 2
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-2">
        <div className="flex-1 min-w-0">
          {isArchived ? (
            <div className="text-xs text-gray-400 dark:text-gray-500 py-2">{t('habits.groups.archived')}</div>
          ) : hasValueTarget ? (
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                <span
                  className={`text-xs font-medium ${targetMet ? 'text-green-500' : 'text-gray-600 dark:text-gray-400'}`}
                >
...
export default function HabitsPage() {
  ...
  const handleCheckIn = (habitId: string, value?: number) => {
    ...
    checkIn.mutate({ date, habitId, value })
  }
  ...
  const handleGroupContextMenu = (e: React.MouseEvent, group: HabitGroup) => {
    e.preventDefault()
    setContextMenu({ group, type: 'group', x: e.clientX, y: e.clientY })
  }

  const handleHabitContextMenu = (e: React.MouseEvent, habit: Habit) => {
    e.preventDefault()
    setContextMenu({ habit, type: 'habit', x: e.clientX, y: e.clientY })
  }
  ...
  const smartGroups = useMemo(
    () => [
      { count: habits.length, icon: '⭐', id: 'all', labelKey: 'habits.groups.all' },
      { count: 0, icon: '📅', id: 'week', labelKey: 'habits.groups.week' },
      { count: archivedHabits.length, icon: '📦', id: 'archived', labelKey: 'habits.groups.archived' },
      { count: 0, icon: '🗑️', id: 'deleted', labelKey: 'habits.groups.deleted' },
    ],
  ...
  return (
    <div className="flex-1 flex overflow-hidden">
      {/* Groups Panel */}
      <div
        style={{ backgroundColor: 'var(--theme-bg-20)', width: habitGroupsPanelWidth }}
        className="shrink-0 border-r border-gray-200 dark:border-gray-700 overflow-auto"
      >
        {/* Smart Groups */}
            ...
            {smartGroups.map((sg) => {
              ...
              return (
                ...
                >
                  ...
                  {sg.count > 0 && <span className="text-xs text-gray-400 dark:text-gray-500">{sg.count}</span>}
                </button>
              )
            })}
          </div>
      ...
      <div className="flex-1 overflow-auto px-8 py-4" style={{ backgroundColor: 'var(--theme-bg-2)' }}>
        ...
        {editingGroup && (
          <div className="fixed inset-0 z-50 flex items-center justify-center">
            <div className="absolute inset-0 bg-black/50" onMouseDown={() => setEditingGroup(null)} />
