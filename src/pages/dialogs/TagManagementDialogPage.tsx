...
const PRESET_COLORS = ['#EF4444', '#F59E0B', '#10B981', '#3B82F6', '#8B5CF6', '#EC4899', '#6366F1', '#14B8A6']
  ...
  return (
    ...
    >
      <button
        ...
        className="p-0.5 rounded hover:bg-gray-200 dark:hover:bg-gray-700 opacity-0 group-hover:opacity-100 transition-opacity cursor-grab active:cursor-grabbing shrink-0 touch-none"
      ...
      {/* Color chip */}
      <span className="w-3 h-3 rounded-sm shrink-0" style={{ backgroundColor: tag.color || '#3B82F6' }} />

      <span className="text-sm text-gray-900 dark:text-gray-100 flex-1 truncate">{tag.name}</span>

      {count > 0 && (
        <span className="text-[10px] text-gray-400 dark:text-gray-500 bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded-full shrink-0">
          {count}
        </span>
      )}
    </div>
...
function TagEditPanel({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!name.trim()) return
    if (isEditing && tag) {
      updateTag.mutate({ color, emoji, id: tag.id, name: name.trim() }, { onSuccess: onClose })
    } else {
      createTag.mutate(
        {
          color,
  ...
  return (
    <div className="flex-1 flex flex-col overflow-auto">
      <div
        data-tauri-drag-region
        className="flex items-center justify-between px-2 py-2 border-gray-200 dark:border-gray-700 shrink-0"
      >
      ...
      <form className="p-4 space-y-4 flex-1">
        ...
        <div className="items-center gap-2 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
          <label
            className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5"
            htmlFor="tag preview"
            aria-labelledby="tag preview"
          >
            {t('tags.preview')}
          </label>
          <span
            className="px-2 py-1.5 pe-4 rounded-full text-sm text-white font-medium"
            style={{
              backgroundColor: color,
              color: `hsl(from ${color} h s abs(calc(calc(l - 40) + min(0, calc(l - 40)) * -1 + 100 * max(0, -1 * calc(l - 40)) / calc(l - 40)))`,
            }}
          >
...
export default function TagManagementDialogPage() {
  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      if (newLevel > overTag.level) {
        ...
        for (let i = 0; i < overTag.level - newLevel && target; i++) {
      ...
      const moves: { id: string; parentId?: string | null; level?: number; sortOrder?: number }[] = [
        { id: activeId, level: effectiveLevel, parentId: newParentId ?? '', sortOrder },
      ]
      for (const desc of descendants) {
        moves.push({ id: desc.id, level: desc.level + levelDelta })
      }
  ...
  const canMoveForward = useCallback(
    (tag: Tag): boolean => {
      const siblings = getSiblings(tag)
      return siblings.findIndex((s) => s.id === tag.id) > 0
    },
    [getSiblings],
  )
  ...
  const handlePromote = useCallback(
    (tag: Tag) => {
      ...
      moveTags.mutate([{ id: tag.id, level: newLevel, parentId: grandParentId }])
    },
    [tags, moveTags],
  )
  ...
  const handleDelete = (tag: Tag) => {
    ...
    const msg =
      children.length > 0
        ? t('tags.delete_confirm_with_children', { count: children.length, name: tag.name })
