    ...
    <div className="flex-1 flex flex-col overflow-auto">
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-5 flex-1">
            ...
            {PRESET_COLORS.map((c) => (
              <button
                ...
                className={`w-9 h-9 rounded-full transition-all ${
                  color === c ? 'ring-2 ring-offset-2 ring-blue-500 dark:ring-offset-gray-800' : 'hover:scale-110'
                }`}
        ...
        <div className="flex items-center gap-2 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
          <span className="text-lg">{emoji}</span>
          <span className="px-2.5 py-1 rounded-full text-sm text-white font-medium" style={{ backgroundColor: color }}>
            {name || t('tags.preview')}
          </span>
        </div>
...
export default function TagsPage() {
  ...
  return (
    <div className="flex-1 flex overflow-hidden">
      {/* Left panel: Tag tree */}
      <div className="w-80 flex flex-col border-r border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shrink-0">
        {/* Header */}
