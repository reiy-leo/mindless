...
const SMART_GROUPS: { id: SmartGroupId; icon: React.ComponentType<{ className?: string }>; labelKey: string }[] = [
  { icon: Star, id: 'favorites', labelKey: 'notes.smart_groups.favorites' },
  { icon: BookOpen, id: 'all', labelKey: 'notes.smart_groups.all' },
  { icon: Archive, id: 'archived', labelKey: 'notes.smart_groups.archived' },
  { icon: CheckCircle, id: 'completed', labelKey: 'notes.smart_groups.completed' },
]

export default function NotesPage() {
  const { t } = useTranslation('common')
  const {
    noteGroupsPanelWidth,
    detailPanelWidth,
    setNoteGroupsPanelWidth,
    setDetailPanelWidth,
    dateFormat,
    timeFormat,
  } = useAppStore()
  ...
  const linkedTaskIds = useMemo(
    () => linkedItemsData.filter((li) => li.linkedType === 'task').map((li) => li.linkedId),
    [linkedItemsData],
  )
  ...
  const linkedMediaItems = useMemo(
    () => allMediaItems.filter((m) => linkedMediaIds.includes(m.id)),
    [allMediaItems, linkedMediaIds],
  )

  const handleLinkItem = useCallback(
    (linkedType: string, linkedId: string) => {
      if (!selectedNoteId) return
      linkNoteItem.mutate({ linkedId, linkedType, noteId: selectedNoteId })
    },
  ...
  const selectedNote = useMemo(() => allNotes.find((n) => n.id === selectedNoteId) || null, [allNotes, selectedNoteId])
  ...
  const handleContentChange = useCallback(
    (content: string) => {
      ...
      contentDebounceRef.current = setTimeout(() => {
        updateNote.mutate({ content, id: selectedNoteId })
      }, 500)
    },
  ...
  const flatItems = useMemo<FlatItem[]>(() => {
    ...
    filteredNotes.forEach((note) => {
      ...
      subs.forEach((sub) => {
        items.push({ parentNote: note, subnote: sub, type: 'subnote' })
      })
    })
    return items
  ...
  const handleNewNote = useCallback(() => {
    createNote.mutate(
      { groupId: selectedGroupId || undefined, title: '' },
  ...
  return (
    <div className="flex h-full bg-gray-50 dark:bg-gray-900" ref={containerRef}>
            ...
            {groupsExpanded && (
              <>
                {noteGroups.filter((g) => !g.isArchived).length === 0 && !showGroupForm && (
                  <p className="text-xs text-gray-400 dark:text-gray-500 px-2.5 py-2">{t('notes.groups.no_groups')}</p>
                )}
                {noteGroups
      ...
      <ResizeHandle onResize={(delta) => setNoteGroupsPanelWidth((w) => Math.max(215, Math.min(315, w + delta)))} />

      {/* Middle Panel: Notes List */}
      <div
        className="flex flex-col min-w-62.5 max-w-87.5"
                  ...
                  >
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        >
                          {note.isPinned && <Star className="w-3 h-3 text-yellow-500 fill-yellow-500 shrink-0" />}
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="flex-1 overflow-auto min-h-0">
                    ...
                    <div className="mt-1 space-y-1">
                      {linkedTasks.map((task) => {
                        const linkItem = linkedItemsData.find(
                          (li) => li.linkedType === 'task' && li.linkedId === task.id,
                        )
                        return (
                          <div key={task.id} className="flex items-center gap-2 group text-sm">
                            <span className="text-gray-400 dark:text-gray-500 shrink-0 w-30">
                            ...
                            {linkItem && (
                              <button
                                ...
                                className="opacity-0 group-hover:opacity-100 hover:text-red-500 text-gray-400 shrink-0"
                              >
                                <X className="w-3 h-3" />
                              </button>
                            )}
                          ...
                          <div key={person.id} className="relative group flex flex-col items-center gap-1 p-2">
                            {linkItem && (
                              <button
                                type="button"
                                onClick={() => handleUnlinkItem(linkItem.id)}
                                className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 hover:text-red-500 text-gray-400"
                              >
                                <X className="w-3 h-3" />
                              </button>
                            )}
                  ...
                  {linkedMediaItems.length > 0 && (
                    <div className="mt-1 flex flex-row gap-2">
                      {linkedMediaItems.map((media) => {
                        const linkItem = linkedItemsData.find(
                          (li) => li.linkedType === 'media' && li.linkedId === media.id,
                        )
                        return (
                          <div key={media.id} className="relative group flex flex-col overflow-hidden">
                            ...
                            <div className="p-1.5">
                              ...
                              {media.year && (
                                <p className="text-[10px] text-gray-400 dark:text-gray-500">{media.year}</p>
                              )}
                            </div>
                          </div>
                        )
              ...
              <div className="px-4 py-2 border-t border-gray-200 dark:border-gray-700 text-[10px] text-gray-400 dark:text-gray-500">
                <p>
                  {t('common.edit')}: {formatDisplayDate(selectedNote.updatedAt.slice(0, 10), dateFormat, t)}{' '}
                  {formatTime(selectedNote.updatedAt.slice(11, 16), timeFormat)}
                </p>
              </div>
            </div>
          </div>
      ...
      {noteContextMenu && (
        <>
          <div className="fixed inset-0 z-50" onMouseDown={() => setNoteContextMenu(null)} />
          <div
            className="fixed z-50 bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 py-1 min-w-40"
